...
#define FORD(g,i,n) for(int i = n-1; i >= g; i--)
...
int subramanianbalaji;
int freq[200005], f[200005];
int32_t main()
{
	cin >> subramanianbalaji;
	while(subramanianbalaji--)
	{
		int n;
		cin >> n;
		fill(freq, freq+n+5, 0);
		fill(f, f+n+5, 0);
		FOR(0, i, n)
		{
			int k;
		...
		sort(f, f+n+3);
		reverse(f, f+n+3);
		int maxf = 0, maxable = f[0];
		FOR(0, k, n)
		{
			if(f[k] == 0) break;
			if(f[k] > 0)
			{
				if(f[k] > maxable) f[k] = maxable;
				freq[f[k]]++;
				maxable = f[k]-1;
			}
			maxf = max(maxf, f[k]);
		}
		// FOR(1, i, maxf+1) cout << freq[i] << endl;
		int msum = 0;
		FORD(1, i, maxf+1)
		{
			if(freq[i]) msum += i;
